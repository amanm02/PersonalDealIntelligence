...
AgentOps is the continuous improvement layer for repository agents working on Personal Deal Intelligence.

## Goal

Make the repository more agent-legible, verifiable, safe, and self-improving while preserving the Banking MVP scope and local-first safety boundaries.
...
-> open small PR
-> verify
-> update docs, memory, skills, hooks, schemas, or tests
-> repeat only within bounded, reviewable loops
```
...
|---|---|
| Agent forgets repo conventions | Update `AGENTS.md` or scoped docs |
...
| Agent skips verification | Add hook, PR checklist, CI gate, or verification doc update |
